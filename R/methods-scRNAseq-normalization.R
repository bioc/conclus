 .annotateGenes <- function(countMatrix, species="mmu",
    ...
    if (!databaseDir){
        ...
        if (missing(species) & (missing(genomeAnnot) | 
                                missing(ensemblPattern))){
            message(strwrap(prefix=" ", initial="", 
            "Species is either not selected or not equal to 'mmu' or 'human'.
			Please, select among default species or use options genomeAnnot and
			ensemblPattern.
            See example in the help page.\n"))
            return(NULL)
        ...
        ## if none of ENSEMBL genes are in database 
        ## we fill their rowData rows with NA
        ...
        message("Annotating rest ", length(symbolGenes), 
                " genes onsidering them as SYMBOLs.")
        ...
        rowdata_GO <- rowdata_GO[!is.na(rowdata_GO$name_1006) &
                            ...
                            b(rowdata_GO$name_1006 == strwrap("cell surface
                                                            receptor signaling
                                                            pathway"))), ]
        ...
        rowdata <- merge(rowdata, 
                         rowdata_GO[c("nameInCountMatrix",
                                      "go_id",
                                      "name_1006")],
                         by.x="nameInCountMatrix",
                         by.y="nameInCountMatrix",
                         all.x=TRUE,
                         all.y=TRUE,
                         sort=FALSE)
        ...
        message(strwrap(prefix=" ", initial="", paste0("Annotating rest ", 
        length(symbolGenes), " genes
...
.filterCells <- function(countMatrix, colData, genesSumThr=100,
    ...
    for(j in 1:length(mb)){
        ...
        reportTable[colData[,colnames(colData) == mb[j]] >= 
    ...
    for(j in 1:length(mw)){
        ...
        reportTable[colData[,colnames(colData) == mw[j]] <= 
                        as.numeric(threshold),
...
.addCellsInfo <- function(countMatrix, rowData, colData=NULL){
    ...
    coldata <- dplyr::mutate(coldata,
                             mtPer    =100 * coldata$mtGenes  
                                         / coldata$genesNum,
                             codPer   =100 * coldata$codGenes 
                                         / coldata$genesNum,
                             sumMtPer =100 * coldata$mtSum    
                                         / coldata$genesSum,
                             sumCodPer=100 * coldata$codSum   
                                         / coldata$genesSum)
    rm(mt, cod)
...
setMethod(
    ...
    definition=function(theObject,
        ...
        colData <- .addCellsInfo(countMatrix, 
                                 rowData=rowData, 
                                 colData=colData)
