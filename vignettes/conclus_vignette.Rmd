...
author: | 
  | Ilyess Rachedi$*, Polina Pavlovich$^1$$^2$$^3$, Nicolas Descostes$^1$, and Christophe Lancrin$^1$
  |
...
normMat <- SingleCellExperiment::counts(SCEobject)
...
Now we re-calculate the markers for each cluster:
\newline
```{r addClusteringManually_runCONCLUS, results = "hide", message = FALSE, warning = FALSE, fig.keep = 'none'}


setCellsSimilarityMatrix(scrUpdated) <-  matrix(nrow = 1, ncol = 1, dimnames = list("c1", "c1"), data = 1)
setClustersSimilarityMatrix(scrUpdated) <-  matrix(nrow = 1, ncol = 1, dimnames = list("1", "1"), data = 1)
setMarkerGenesList(scrUpdated) <- list(data.frame(Gene = c("gene1"), mean_log10_fdr = c(NA), n_05 = c(NA), score = c(NA)))
setClustersMarkers(scrUpdated) <- data.frame(geneName="gene1", clusters=NA)
setGenesInfos(scrUpdated) <- data.frame(uniprot_gn_symbol=c("symbol"), clusters="1", external_gene_name="gene", go_id="GO1,GO2", 
                ...
                uniprot_gn_id="ID")
setClustersSimiliratyOrdered(scrUpdated) <- factor(1)



scrUpdated <- clusterCellsInternal(scrUpdated, clusterNumber = 9)
scrUpdated <- calculateClustersSimilarity(scrUpdated)
scrUpdated <- rankGenes(scrUpdated)
scrUpdated <- retrieveTopClustersMarkers(scrUpdated, removeDuplicates=FALSE)
scrUpdated <- retrieveGenesInfo(scrUpdated)
...
plotCellHeatmap(scrUpdated, orderClusters=TRUE, orderGenes=TRUE, meanCentered=FALSE)
plotGeneExpression(scrUpdated, "Ccl3")
plotGeneExpression(scrUpdated, "Gp9")
plotGeneExpression(scrUpdated, "Fn1")
