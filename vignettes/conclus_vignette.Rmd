...
Issues can be submitted directly on the Bioconductor forum using the keyword 
...
*Tal1*, *Fli1*, *Lyl1*, *Erg* and *Lmo2* in *in vitro* differentiated 
embryonic stem cells (ESCs) in comparison to control. They knocked-in a 
polycistronic transgenic construct allowing to over-express eigth 
transcription factors (i8TFs) simultaneously after adding doxycycline (dox). 
The Empty ESC line did not have any transgene. There were **four conditions**: 
E_minus (Empty no dox), E_plus (Empty with dox), i8TFs_minus (i8TFs no dox) 
and i8TFs_plus (i8TFs with dox). 
...
(PCA), and they found that there was a major gene expression difference 
between i8TFs_plus and the other three conditions (*Figure 4—figure supplement 
2*). **However, it was not clear if other sub-clusters could be identified 
...
*Tal1*, *Fli1*, *Lyl1*, *Erg*, and *Lmo2*. Highly abundant embryonic 
hemoglobins with names starting with "Hba" or "Hbb" were also excluded because 
...
[GSE96982](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE96982). The 
URL of the count matrix was retrieved by right-click and 'copy link adress' 
on the ftp hyperlink of the supplementary file GSE96982_countMatrix.txt.gz 
at the bottom of the page. The name of the series matrix, containing 
columns meta-data was retrieved by clicking the 'Series Matrix File(s)' link 
just above the count matrix. Another complete tutorial is given for retrieving 
and processing data of tabulamuris on the [landing page]() of the package on 
the Bioconductor website. The function `retrieveFromGEO` will download all 
series matrices present in the GEO record however only the one of interest will 
be kept.
...
## Removing the 8 TFs:
TFtoRemove <- c("Runx1", "Cbfb", "Gata2", "Tal1", "Fli1", "Lyl1", "Erg", 
"Lmo2")
...
idxHuman <- which(columnsMetaData$state == "Pos_Ctrl")
columnsMetaData <- columnsMetaData[-idxHuman,]
countMatrix <- countMatrix[,-idxHuman]
```
...
![Knn plot. If the dashed horizontal line in the k-NN distance plot lays on 
the “knee” of the curve, it means that optimal epsilon is equal to the 
intersection of the line to the y-axis.](figures/1_distancegraph.png)

```{r testClustering_result1, results="hide", fig.keep = 'none'}
# saved as "outputDirectory/test_clustering/test_tSNE.pdf"
p[[1]]
```
![tsne One of the 14 tSNE (by default) generated by 
...
p[[2]]
```
![clusterplot One of the 84 dbscan clustering solutions generated by 
...
CONCLUS uses [Scran](https://bioconductor.org/packages/release/bioc/html/
scran.html) and [Scater](http://www.bioconductor.org/packages/release/bioc/
html/scater.html) packages for normalization. Beforehand, the 
...
![tSNEclusters DBscan results on the 5th tSNE.](figures/4_tSNEclusters.png)
...
![tSNEnoColor The 5th tSNE solution without 
coloring.](figures/5_tSNEnoColor.png)
...
![tSNEstate The 5th tSNE solution colored by cell 
condition.](figures/6_tSNEstate.png)
...
![cellSim Cell similarity matrix showing the conservation of clustering 
across the 84 solutions.](figures/7_cellSimilarity.png)
...
evidence showing that CONCLUS managed not only to distinguish i8TFs_plus cells 
from the three other groups (as in the original publication) but also find 
...
![clustersim Cluster similarity matrix. Each cell belonging to a particular 
cluster were merged into a pseudo-cell.](figures/8_clustersim.png)
...
clustering solutions. Only cluster 5 has a quite low similarity value. 
Red color on the diagonal means that the group is homogenous, and usually, it 
is what we want to get. The yellow on the diagonal indicates that either that 
...
![cellHeatmap Heatmap showing the expression of the top 10 markers for each 
cluster. The values are normalized according to the 
mean.](figures/9_cellHeatmap.png)
...
at a level of seven and nine look very similar. It is hard to highlight all 
the differences in expression of both lowly and highly detected genes in one 
...
```
![cellHeatmapNoMean Same heatmap as before without normalizing by the 
...
```
![Hp Marker gene of cluster 2.](figures/11_Hp_cluster2.png)

```{r plotGeneExpression2, eval=FALSE}
plotGeneExpression(scr, "Alox5ap", tSNEpicture=5)
```

![Alox5ap Marker gene of cluster 9.](figures/12_Alox5ap_cluster9.png)
...
![CD52 Marker gene of cluster 9.](figures/13_CD52_cluster9.png)
...
It is mostly obvious for clusters 2 and 3. In order to figure out what marker 
genes are defining these families of clusters, one can use manual clustering in 
CONCLUS to fuse clusters of similar nature: i.e. combine clusters 2 and 3 
together.
...
```{r addClusteringManually_visualization1, results="hide", fig.keep = 'none'}
plotClustersSimilarity(scrUpdated)
```
![updatedClustSim Updated clusters similarity matrix with cluster 2  
representing the merged old clusters 2 and 
3.](figures/14_updatedClusterSim.png)
...
![updatedCellSim Updated cells similarity matrix with cluster 2 representing 
the merged old clusters 2 and 3.](figures/15_updatedCellSim.png)
...
![updatedCellHeatmap Updated cells heatmap with cluster 2 representing the 
merged old clusters 2 and 3.](figures/16_updatedCellHeatmap.png)
...
![updatedTSNE 5th tSNE solution colored by dbscan result showing the cluster 2 
as being the merge of the old clusters 2 and 3.](figures/17_updatedTSNE.png)


The cell heatmap above shows that Tyrobp and Ccl9 are good markers of cluster 
2 (at the bottom). One can visualize them in the t-SNE plots below.

```{r replot1, eval=FALSE}
plotGeneExpression(scrUpdated, "Tyrobp", tSNEpicture=5)
```

![Tyrobp Marker gene for cluster 2 (mix of old clusters 2 and 
3)](figures/18_Tyrobp_cluster2.png)
...
![Ccl9 Marker gene for clusters 2 (mix of old clusters 2 and 
3)](figures/19_ccl9_cluster2.png)


# Conclusion

Here we demonstrated how to use CONCLUS and combine multiple parameters 
testing for sc-RNA-seq analysis. It allowed us to gain more information on the 
dataset of Bergiers et al and will help gaining deeper insights into others. 
