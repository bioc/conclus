...
plotGeneExpression(scr, "Ccl3", tSNEpicture=5)
```


```{r plotGeneExpression3}
# Fn1, marker gene for cluster 7
plotGeneExpression(scr, "Fn1", tSNEpicture=5)
```
...
It is mostly obvious for clusters 5 and 6. In order to figure out what marker 
genes are defining these families of clusters, one can use manual clustering in 
CONCLUS to fuse clusters of similar nature: i.e. combine clusters 5 and 6 
together.
...
## Replace "6" by "5" to merge 5/6
clustCellsDf$clusters[which(clustCellsDf$clusters == 6)] <- 5
