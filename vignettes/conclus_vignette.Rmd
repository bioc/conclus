...
Due to the stochastic aspect of the tSNE, images of the plot are directly 
included (and not generated) to make the descriptions consistent. You might 
therefore get slightly different plots. However, you should obtain the same 
marker genes at the end of the process.
...
In the example below, a small toy example is taken to illustrate the 
...
coloration. You will also find sub-folders containing: 
    ...
    heatmap.
    + `5_ClusterSimilarityMatrix`: The matrix underlying the clusters 
    similarity heatmap.
...
2*). **However, it was not clear if other subclusters could be identified 
...
sries matrices present in the GEO record however only the one of interest will 
be kept.
...
pair of cells were in one cluster (score between 0 and 1, between blue and 
...
clustering solutions. Only cluster 6 have a quite low similarity value. Red 
...
particular cluster compared to the others (multiple comparisons). The function 
*rankGenes* performs multiple comparisons of all genes from theObject and rank 
them according to a score reflecting a FDR power.
...
Following the execution of the `retrieveTopClustersMarkers`, CONCLUS offers 
the option to visualize the marker genes on a heatmap. Below we chose to show 
the selected 10 marker genes per cluster which should generate a heatmap with 
100 genes (10 marker genes x 10 clusters). This is convenient for 
...
```{r plotCellHeatmap_marker1, fig.height=5, fig.width=6}
...
```{r plotCellHeatmap_marker2, fig.height=5, fig.width=6}
...
# Srgn, marker gene
plotGeneExpression(scr, "Srgn", tSNEpicture=5)
```


```{r plotGeneExpression3}
# Hba-a2, marker gene
plotGeneExpression(scr, "Hba-a2", tSNEpicture=5)
```
...
It is mostly obvious for clusters 7 and 3. In order to figure out what marker 
genes are defining these families of clusters, one can use manual clustering in 
CONCLUS to fuse clusters of similar nature: i.e. combine clusters 7 and 3 
together.
...
## Replace "3" by "7" to merge 3/7
clustCellsDf$clusters[which(clustCellsDf$clusters == 3)] <- 7
...
```{r addClusteringManually_visualization3, fig.height=5, fig.width=6}
plotCellHeatmap(scrUpdated, orderClusters=TRUE, orderGenes=TRUE)
```

This heatmap shows that Cd52 and Alox5ap are good markers of cluster 5 (mix of 
old clusters 5 and 6). We can visualize them in the t-SNE plots below.
...
plotGeneExpression(scrUpdated, "Cd52", tSNEpicture=5)
```

```{r replot2}
# Alox5ap, marker gene for cluster 9 (mix of old clusters 9 and 10)
plotGeneExpression(scrUpdated, "Alox5ap", tSNEpicture=5)
```
