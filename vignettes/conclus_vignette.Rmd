...
development. The YS major component was an epithelial population expressing a 
rare population of liver marker genes. This vignette aims at reproducing the 
identificaton of the liver like population in the YS at day 11.5. The number 
...
and normalization, 1303 cells are retained.  
...
just above the count matrix. The function `retrieveFromGEO` will download all 
...
set *runQuickCluster = FALSE*, by default it is *TRUE*. We advice to first try 
the analysis with this option and to set it to FALSE if no rare populations 
are found. 
...
Note that the number of clusters is set to 10. Considering that clusters can 
...
Cluster 8 was identified as being a rare population with markers being 
specific of liver. Below are examples of these marker genes:
