...
The code below format the count matrix and the columns meta-data:
...
download.file("https://www.ncbi.nlm.nih.gov/geo/download/?acc=GSE96982&format=
file&file=GSE96982%5FcountMatrix%2Etxt%2Egz", countMatrixPath)
countMatrix <- as.matrix(read.table(countMatrixPath, header=TRUE, row.names=1,
                                stringsAsFactors = FALSE))

## Retrieving the columns meta-data
gpl <- GEOquery::getGEO(filename="GSE96982-GPL19057_series_matrix.txt.gz")
...
## Removing the 8 TFs:
TFtoRemove <- c(Runx1 = "ENSMUSG00000022952", 
Cbfb="ENSMUSG00000031885",
Gata2="ENSMUSG00000015053",
Tal1="ENSMUSG00000028717",
Fli1="ENSMUSG00000016087",
Lyl1="ENSMUSG00000034041",
Erg="ENSMUSG00000040732",
Lmo2="ENSMUSG00000032698")
...
symbols <- clusterProfiler::bitr(matrixSym, fromType="ENSEMBL", 
                                toType=c("SYMBOL"), OrgDb= "org.Mm.eg.db")
idxSymRemove <- which(duplicated(symbols$ENSEMBL))
symbols <- symbols[-idxSymRemove,]
idxSym <- match(symbols$ENSEMBL, matrixSym)
rownames(countMatrix)[idxSym] <- symbols$SYMBOL  
```
